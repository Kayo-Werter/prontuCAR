    ...
    return (
        <div className="p-6 grid justify-items-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-2 w-full max-w-md">
                ...
                <div>
                    <label className="block text-sm font-medium">Litros de Combutível</label>
                    <input
                        type="number"
                        name="liters_gasoline"
                        value={formData.liters_gasoline}
...
const NovoAbastecimento = () => {
    ...
    const handlePriceGasolineChange = (value: string | undefined) => {
        const formattedValue = value ? formatValue(value) : "";
        setFormData(prevFormData => {
            const updatedData = { ...prevFormData, price_gasoline: formattedValue };
            const price_gasoline = parseFloat(updatedData.price_gasoline.replace(/\./g, '').replace(',', '.'));
            const value_total = parseFloat(updatedData.value_total.replace(/\./g, '').replace(',', '.'));
            if (price_gasoline > 0 && value_total > 0) {
                updatedData.liters_gasoline = (value_total / price_gasoline).toFixed(2);
            } else {
                updatedData.liters_gasoline = "";
    ...
    const handleValueTotalChange = (value: string | undefined) => {
        const formattedValue = value ? formatValue(value) : "";
        setFormData(prevFormData => {
            const updatedData = { ...prevFormData, value_total: formattedValue };
            const price_gasoline = parseFloat(updatedData.price_gasoline.replace(/\./g, '').replace(',', '.'));
            const value_total = parseFloat(updatedData.value_total.replace(/\./g, '').replace(',', '.'));
            if (price_gasoline > 0 && value_total > 0) {
                updatedData.liters_gasoline = (value_total / price_gasoline).toFixed(2);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const refuelData = {
            ...
            price_gasoline: parseFloat(formData.price_gasoline.replace(/\./g, '').replace(',', '.')),
            value_total: parseFloat(formData.value_total.replace(/\./g, '').replace(',', '.')),
            liters_gasoline: parseFloat(formData.liters_gasoline),
            ...
            <form onSubmit={handleSubmit} className="space-y-2 w-full max-w-md">
                ...
                <div>
                    ...
                    <CurrencyInput
                        name="price_gasoline"
                        value={formData.price_gasoline}
                        onValueChange={handlePriceGasolineChange}
                        prefix="R$ "
                        placeholder="R$ 0,00"
                        className="w-full p-2 border rounded"
                        decimalsLimit={2}
                ...
                <div className="text-center">
                    ...
                    <button className="mt-4 w-full border-4 border-blue-600 text-blue-600 px-4 py-2 rounded" type="button" onClick={() => router.push('/historicoAbastecimento')}>
                        Cancelar
                    </button>
                </div>
            </form>
